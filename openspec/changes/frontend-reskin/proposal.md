...
### New Capabilities
(none — this reskins an existing capability's UI, it does not add new backend-facing behavior)
...
- **Affected code**: `frontend/src/App.jsx`, `frontend/src/App.css`, `frontend/src/index.css`, and all of `frontend/src/components/*` (rewritten/replaced); their corresponding `*.test.jsx` files are rewritten for the new structure. No changes to `frontend/src/api/client.js` or any `backend/` code.
...
- **No API/schema changes** — consumes the existing `GET /api/jurisdictions`, `GET /api/jurisdictions/{id}/legislators`, and `GET /api/jurisdictions/{id}/party-summary` endpoints as-is.
